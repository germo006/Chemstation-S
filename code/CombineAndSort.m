...
% 08 January 2024: NPG using a *severely hacked* version of this script to
% perform the same sort of analysis, but on only one isotope and two ion
% modes. 
...
clear
clc
load("H:\2022_0214_AE2123_BC\Chemstation-S\datasets/Output_Curves/AE2123_NPG_curve34.2024.01.08_C13.mat")
% load("SkyMat_testing_3isotopes.2023.12.19_D5.mat")

outdir = "H:\2022_0214_AE2123_BC\Chemstation-S\datasets/";
filename = "AE2123_NPG_curve34.2024.01.08_OneMode.mat";
...
% both isotopes.
mtabNames_both = mtabNames_C13;
mtabNames_all = mtabNames_C13;
...
for ii = 1:length(mtabNames_all)
    ...
    if ~ismember(mtabNames_all(ii),mtabNames_both) && ismember(mtabNames_all(ii),mtabNames_C13)
        ...
        if 0
            ...
            exportgraphics(gca, "H:/2022_0214_AE2123_BC/Chemstation-S/datasets/PredictionIntervals.pdf", 'Append',  true)
            hold off
            close(gcf)
...
for ii= 1:length(uniqueNames)
    ...
    if isempty(iallp)
        ...
        LOQ_OneMode(ii) = LOQ(ialln);
        LOD_OneMode(ii) = LOD(ialln);
        var_OneMode(ii,:) = PI95(ialln,:);
        modeUsed(ii,:) = "-";
        continue
    elseif isempty(ialln)
        mtabData_OneMode(ii,:) = mtabData_all(iallp,:);
        LOQ_OneMode(ii) = LOQ(iallp);
        LOD_OneMode(ii) = LOD(iallp);
    ...
    LOQ_OneMode(ii) = min([LOQ(iallp),LOQ(ialln)]);
    LOD_OneMode(ii) = min([LOD(iallp),LOD(ialln)]);
